...
**ACTION ITEM (added 2026-07-18) — support general, self-directed learning, not just exams/certs.** Krakenote should work just as well for open-ended, ongoing learning where there's no test date — e.g., someone learning to code who keeps running notes on what they're picking up. That user wants to capture notes over time, consolidate/summarize them, build durable recall, and revisit later. Implications to weave in when scoping: (1) support "living" note collections that grow over time, not just one-shot capture → cards; (2) let users organize by topic/subject rather than only by exam; (3) the spaced-repetition + [[audio study mode]] loops apply to skill-learning notes just as well as to exam decks; (4) reflect this in positioning/onboarding so it reads "study anything you're learning," while still *marketing* first to the deadline-driven exam/cert segment for conversion. This broadens the use case without changing the launch go-to-market.
