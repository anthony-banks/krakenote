-- KRA-96: analytics_summary(days) — aggregates the events table for the admin
-- dashboard. Runs entirely in Postgres (efficient). Locked to the service role:
-- only the server (superuser-gated /api/admin/analytics) may call it, never a
...
as $$
  ...
  select jsonb_build_object(
    ...
    'funnel', jsonb_build_object(
      ...
      'activated_users',   (select count(distinct user_id) from ev where name = 'ai_generation' and user_id is not null),
      'paywall_shown',     (select count(*) from ev where name = 'paywall_shown'),
      'upgrade_started',   (select count(*) from ev where name = 'upgrade_started'),
